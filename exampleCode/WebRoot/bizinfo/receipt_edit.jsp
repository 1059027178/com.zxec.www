...
<html>
  ...
  <script type="text/javascript">
  	function forward(){
  	window.location.href="MainServlet?flag=1.1";
  }
  </script>
  <body>
  <div>
	...
	<input name="unit" 	   type="hidden" value="<%=request.getSession().getAttribute("unit")== null ? "" : request.getSession().getAttribute("unit") 	%>" />
  		<ul>
  			<li class="li" style="line-height:30px;">
  				返回消息：<%=request.getSession().getAttribute("type") == null ? "" : request.getSession().getAttribute("type")%>
  			</li>
  			...
  			<%-- <c:if test="${type!='E'}"><input type="submit" style="width:40px;height:25px;" class="button" value="上架"/></c:if> --%>
  			<input type="submit" style="width:40px;height:25px;" class="button" value="上架"/>
