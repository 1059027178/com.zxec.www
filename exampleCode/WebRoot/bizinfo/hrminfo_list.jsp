      ...
      <div class="row-fluid">
		...
		$('#myModal').on('hidden', function() {
		  alert();
			...
			<a class="btn btn-medium btn-primary" href="/hrminfoAdd.do">新建人员信息</a><br/><br/>
			
			<display:table name="hrminfoList" export="false" class="table table-striped table-bordered" id="list" requestURI="/hrminfoList.do">
				<display:column title="选择" style="width:5%">
					<input name="selId" type="checkbox" value="${list.hrminfoId}" />
				</display:column>
				<display:column title="工号" escapeXml="false" style="width:20%">
					   <a href="/hrminfoEdit.do?hrminfoId=${list.hrminfoId}" >${list.objno}</a>
				    &nbsp;
				    <a href="javascript:;" title="删除" onclick="if(confirm('确定要删除本条记录?')){deleteSingleObj(${list.hrminfoId});}"><i class="icon-remove"></i></a>
			
				</display:column>
				<display:column title="姓名" property="name"/>
				<display:column title="部门" property="department"/>
			...
			function deleteSingleObj(keyId){ 
			  ...
			  //ajax调用  
			    $.ajax({    
			        type:'post',        
			        url:'/hrminfoJson.do?showType=deletehrminfo',    
			        data:{hrminfoId:keyId}, 
		        	dataType:'json',
			        cache:false,
			...
			function deleteMoreObjs(){ 
	          ...
	          if(confirm("确定要删除所选项目？")) {    
	              ...
	              $.ajax({    
	                  type: "POST",    
	                  url: "/hrminfoJson.do?showType=deletehrminfos",    
