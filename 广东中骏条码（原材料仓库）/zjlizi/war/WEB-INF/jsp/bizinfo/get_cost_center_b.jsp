...
<%@ page import="com.thinkway.cms.business.domains.CostDetail"%>
<%
	...
	String costGettenId = (String) request.getAttribute("costGettenId");
%>
...
//过账
	function posting() {
		document.form.action = "/getCostE.do";
...
<%=pageNum%>
	;
		...
		document.form.action = "/getCostB.do";
		document.form.submit();
			} 
	...
	function confirm() {
		...
		for ( var i = 0; i < temp.length; i++) {
			if (temp[i].checked) {
				...
				document.form.action = "/getCostC.do";
				document.form.submit();
				return;
			}
		}
...
<BODY>
	<div class="div">
		<form id="form" name="form" method="post" onsubmit="return false;">
			...
			<%
				List<CostDetail> list = (List) request.getAttribute("costDetailList");
				if (list == null) {
			%>
			...
			<table class="table_list"
				...
				<%
					...
					for (i = 1; i <= list.size(); i++) {
						CostDetail pic = (CostDetail) list.get(i - 1);
				%>
				<tr <%if (i % 2 == 1) {%> class="tr_list_2" <%} else {%>
					class="tr_list_1" <%}%>>
					<td class="td_list"><input type="radio" name="param"
						value="<%=pic.getMaterialId()%>@<%=pic.getBatch()%>@<%=pic.getCostGettenId()%>@<%=pic.getLineItem()%>" /><%=pic.getNum()%></td>
