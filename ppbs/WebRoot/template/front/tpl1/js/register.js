...
$().ready( function() {
		...
		onShow: function(hash){
			if ($.trim($("#agreementContent").html()) == "") {
				$.ajax({
					...
					url: tcrm.base + "/front/member!getAgreement.action",
	...
	function registerWindowCaptchaImageRefresh() {
		$("#registerWindowCaptchaImage").attr("src", tcrm.base + "/captcha.jpg?timestamp" + (new Date()).valueOf());
	}
