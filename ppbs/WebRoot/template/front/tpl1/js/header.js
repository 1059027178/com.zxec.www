...
$().ready( function() {
	...
	$.flushHeaderInfo = function () {
		...
		if($.cookie("loginMemberUsername") != null) {
			$headerLoginMemberUsername.text(" "+$.cookie("loginMemberUsername")+"-"+$.cookie("loginMemberUserdesp"));
	...
	$(".showCartItemList").hover(
		function() {
			...
			if (isAjaxDate) {
				$.ajax({
					url: tcrm.base + "/front/cart_item!ajaxList.action",
					dataType: "json",
					beforeSend: function() {
					...
					success: function(data) {
						...
						$.each(data, function(index, object) {
							if (index == 0) {
								...
								cartItemListHtml = '<li><a class="leftArea" href="' + tcrm.base + object.htmlFilePath + '" title="' + object.name + '">' + object.name.substring(0, 12) + '</a><span class="rightArea"><strong>' + object.price + ' × ' + object.quantity + '</strong></span></li>' + cartItemListHtml;
							}
