...
<%
	...
	String pici=SapUtil.null2String((String)request.getAttribute("pici")); 
%>
...
<BODY>
<div class="div">
	<form  id="listForm" name="listform" method="post" action="/stoquyList.do">
		...
		<%
			...
			%>
				...
				<table style="width:100%;">
					...
					<tr>
							<td align="center" >
							<input type="button" value="返回" style="width:50px;heigth=20%" onclick="window.location.href='/stoquyQuery.do';"></input>
			...
			%>
				<tr <% if(i%2==1){ %>class="tr_list_2"<%}else{ %>class="tr_list_1"<%} %>>
					<td class="td_list"><%=sto.getMatnr() %></td>
					<td class="td_list"  colspan="2"><%=sto.getMaktx() %></td>
					<td class="td_list"><%=sto.getLabst() %><%=sto.getVerme() %></td>
				</tr>
				<tr <% if(i%2==1){ %>class="tr_list_2"<%}else{ %>class="tr_list_1"<%} %>>
					<td class="td_list"><%=sto.getMeins() %></td>
					<td class="td_list"><%=sto.getCharg() %></td>
			...
			<%} %>
			<tr >
				<td align="center" colspan="4"><input class=button type="button" style="width:50px;" value="上一页" onclick="lastPage();"/>
