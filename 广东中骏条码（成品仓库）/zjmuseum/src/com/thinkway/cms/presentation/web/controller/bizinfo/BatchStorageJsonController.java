 ...
 *
 */
public class BatchStorageJsonController extends MultiActionController implements AuthenticateController{
...
public void setAuthenticator(Authenticator authenticator) {
	...
	public ModelAndView checkout(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try{
			...
			parameterList.setValue(ZTMID,"I_ZTMID");
			...
			String EP_ZYSHGZ = SapUtil.null2String(outs.getValue("EP_ZYSHGZ") + "");
			String EP_ZTMID  = SapUtil.null2String(outs.getValue("EP_ZTMID") + "");
			if(null!=myConnection){
				SapUtil.releaseClient(myConnection);
			}
			
			System.out.println("【流水码" + EP_ZTMID + "，入库检验结束：EP_ZYSHGZ(空为未检验) = " + EP_ZYSHGZ + "】");
			response.getWriter().write("{\"ZYFHGZ\": \""+EP_ZYSHGZ+"\"}");
		}catch(Exception e){
			e.printStackTrace();
		}
		return null;
