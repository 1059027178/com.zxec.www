...
public class MatLgpFifoViewController implements Controller , AuthenticateController{
...
//	//从SAP获取交货单数量的值
		...
		String functionName="ZFM_BC_04_31_V1";//函数的名字
			...
			if(ET_LQUA.getNumRows()%3>0){
			 ...
			 for (int i = (Integer.parseInt(currentPage)-1)*3; i < ET_LQUA.getNumRows(); i++) {
				 ET_LQUA.setRow(i);
				    ...
				    delord.setActive(ET_LQUA.getString("ACTIVE"));//Active
				    iList.add(delord);
				    j++;
				    if(j==3){
				    	break;
