...
<%@ page import="com.thinkway.cms.business.domains.GetCost"%>
<%
...
<script>
	function forward() {
		window.location.href = "/getCostA.do";
	}
	function confirm() {
		var temp = document.getElementsByName("costGettenId");
...
<%=pageNum%>
	;
		...
		document.form.action = "/getCostA.do";
		document.form.submit();
	}
	function nextPage() {
		var pageNo =
...
<BODY>
	<div class="div">
		<form name="form" action="/getCostB.do">
			...
			<%
				List<GetCost> list = (List) request.getAttribute("GetCostList");
					if (list == null) {
			%>
			<table style="width:100%;">
				...
				<tr>
					<td align="center"><input type="button" class="button"
						style="width:40px;" value="返回" onclick="window.location.href='/delivery.do?functionType=4';"></input> <input
			...
			<table class="table_list"
				...
				<tr class="tr_list_1" bordercolor="#000000">
					...
					<td class="td_list">成本中心领料单</td>
				</tr>
				<%
					int i = 1;
							for (i = 1; i <= list.size(); i++) {
								GetCost pic = (GetCost) list.get(i - 1);
				%>
				<tr <%if (i % 2 == 1) {%> class="tr_list_2" <%} else {%>
					class="tr_list_1" <%}%>>
					<td class="td_list"><input type="radio"
						name="costGettenId"
						value=<%=SapUtil.null2String(pic.getCostId())%> /></td>
					<td class="td_list"><%=pic.getNum()%></td>
					<td class="td_list"><%=SapUtil.null2String(pic.getCostId())%>
					</td>
				</tr>
			...
			<table style="width:100%;">
				<tr>
					<td align="center"><input type="button" class="button"
						style="width:40px;" value="确认" onclick="confirm()"></td>
					<td align="center"><input type="button" class="button"
						style="width:40px;" value="返回"
						onclick="window.location.href='/delivery.do?functionType=4';">
					</td>
					<td align="center"><input type="button" class="button"
						style="width:40px;" value="<<" onclick="lastPage();" />
					</td>
					<td align="center"><input type="button" class="button"
