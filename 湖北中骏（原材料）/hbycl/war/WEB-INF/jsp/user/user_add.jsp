...
<script>
	$(document).ready(function(){	  
		$.formValidator.initConfig({theme:"Default",formID:"profileForm",debug:false,submitOnce:false,wideword:true,
			...
			ajaxForm:{
				type:'get',        
		        url:'/userJson.do?showType=createUser',
		        data:null,
		        dataType:'json',
		        cache:false, 
		...
		$("#userBH").formValidator({onShow:"请输入用户登录名",onFocus:"用户登录名至少1个字符,最多10个字符"}).inputValidator({min:1,max:10,onError:"您输入的用户名不合法,请确认"}).defaultPassed();
		...
		<span style="padding-left:10px;"> <%=warnmsg1%></span>
		  ...
		  <form class="form-horizontal" id="profileForm">
		      ...
		      <div class="control-group">
			    <label class="control-label" for="userBH">用户登录名</label>
			    <div class="controls">
			      <div class="row">
					  <div class="span3"><input type="text" name="userBH" id="userBH" placeholder="用户登录名">	</div>
					  <div id="userBHTip" class="span4"></div>
					</div>
			    </div>
			  </div>
			  ...
			  <div class="control-group">
			    ...
			    <div class="controls">
				    <div class="row">
					  <div class="span3"><input type="text" name="userName" id="userName" placeholder="用户别名">	</div>
...
function save(){
		...
		jQuery.ajax({
				type:'post',        
		        url:'/userJson.do?showType=createUser',
		         data:{"userBH":userBH,"userName":userName,"inputPassword":inputPassword,"inputPasswordAg":inputPasswordAg,"userFunction":userFunction},
		        dataType:'json',
		        cache:false, 
		        success:function(data){            
			           alert(data.errmsg);   
