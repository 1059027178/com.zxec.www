...
<script>
	function forward() {
		window.location.href = "/getResearchA.do";
	}
	function confirm() {
...
<%=pageNum%>
	;
		...
		document.form.action = "/getResearchA.do";
		document.form.submit();
	}
	function nextPage() {
		var pageNo =
...
<BODY>
	<div class="div">
		<form name="form" action="/getResearchB.do">
			...
			<table class="table_list" style="width:100%;margin-top:40px;margin-left:20px">
				...
				<tr <%if (i % 2 == 1) {%> class="tr_list_2" <%} else {%>
					class="tr_list_1" <%}%>>
					<td class="td_list"><input type="radio" name="materialGettenId"
						value=<%=SapUtil.null2String(pic.getMaterialId())%> />
					</td>
