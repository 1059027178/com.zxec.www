		...
		if ("E".equals(model.getOuts().getStructure("ES_RETURN").getString("MSGTY"))) {
			...
			for (int i = 0; i < jTable.getNumRows(); i++) {
				...
				sapRequest2.addInputTable(
						"IT_DETAIL",
						new String[] { "MRFNR", "SEQNO", "WERKS", "ZZLGORT", "ZZMATNR", "ZZMAKTX", "ZZREMNG",
								"ZZFLORT", "ZZMEINS", "ZZCHARG", "ZZJPMNG" },
						new String[] { jTable.getString("MRFNR"), jTable.getString("SEQNO"),
								SapUtil.null2String(request.getParameter("werks")), jTable.getString("ZZLGORT"),
								jTable.getString("ZZMATNR"), jTable.getString("ZZMAKTX"), jTable.getString("ZZREMNG"),
								jTable.getString("ZZFLORT"), jTable.getString("ZZMEINS"), jTable.getString("ZZCHARG"),
								jTable.getString("ZZJPMNG") });
			}
