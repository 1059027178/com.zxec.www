...
<%
	...
	String requireTotal = (String) request.getAttribute("requireTotal");
...
<BODY>
	<div class="div">
		<form id="form" name="form" action="/getResearchD.do" method="post"
			...
			<table class="table_list"
				...
				<tr>
					<td>需求数量：<label id="requireAmount"><%=requireTotal%></label></td>
				</tr>
					...
					<table style="width:80%;margin-left:20px;margin-right:20px">
						<tr>
							...
							<td align="center"><input type="button" class="button"
								style="width:40px;" value="确认" onclick="confirm()"></input>
								<td align="center"><input type="button" class="button"
									style="width:40px;" value="返回"
									onclick="window.location.href='/delivery.do?functionType=2';"></input>
									<td align="center"><input type="button" class="button"
...
<script>
	...
	pager.paraArr = new Array("num", "wareHouse", "batch", "_turn", "_hidden[class=checkWarehouse]", "enabledAmount",
			...
			if (false) {
				...
				if (!isNaN(num) && num >= 0) {
					if (parseFloat(num) > parseFloat(pager.list[index].enabledAmount)) {
						...
						if (parseFloat(sum) > parseFloat($("#requireAmount").html())) {
							pager.list[index].selectAmount = "0";
							$(this).val("").focus();
							alert("总数超过需求数量！");
						} else {
							$("#sum").val(sum);
